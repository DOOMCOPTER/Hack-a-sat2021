...
void xcrypt(struct AES_ctx *ctx, char *message, uint32_t size)
{
#ifdef DEBUG 
  fprintf(stderr,"Encrypt %d bytes:\nIV Before: ", size);
  for (int ii = 0; ii < AES_BLOCKLEN; ii++)
    fprintf(stderr,"%02x", ctx->Iv[ii]);
  fprintf(stderr,"\n");
#endif 
  AES_CTR_xcrypt_buffer(ctx, (uint8_t *)message, size);

#ifdef DEBUG
  fprintf(stderr,"IV After: ");
  for (int ii = 0; ii < AES_BLOCKLEN; ii++)
    fprintf(stderr,"%02x", ctx->Iv[ii]);
  fprintf(stderr,"\nDone\n");
#endif
